...
import Boundary.UtilityViews.InsertAdminView;
...
public class InsertAdminController {
    ...
    @FXML
    void registraNuovoAdmin(MouseEvent event) {
        ...
        {
                if (ctrl.insertAdminIntoDB(nomeTF.getText(), cognomeTF.getText(), emailTF.getText(), pswdTF.getText(), codiceFTF.getText(), cell1TF.getText(), cell2TF.getText(), birthDatePicker.getValue(), sexChoiceBox.getValue()))
                {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION, "Nuovo amministratore inserito correttamente.", ButtonType.OK);
    ...
    public void hide() {
        InsertAdminView insertAdminView = InsertAdminView.getInstance();
        insertAdminView.hide();
    }
